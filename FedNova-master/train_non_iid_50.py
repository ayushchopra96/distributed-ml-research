...
def run(rank, size):
    ...
    train_loader_list, test_loader_list = get_non_iid_50(
        args.batch_size, 8, size)

    train_loader, test_loader = train_loader_list[rank], test_loader_list[rank]
    train_sizes = np.array(list(train_loader_list[0].ds_sizes.values()))

    logging.debug("Worker id {} local sample ratio {} "
                  "local epoch length {}"
                  .format(rank, train_sizes[rank], len(train_loader)))

    # define neural nets model, criterion, and optimizer
    model = util.ResNet32(10).cuda()
    ...
    for rnd in range(args.rounds):
        ...
        # record metrics
        logging.info("Round {} test accuracy {:.3f} time {:.3f}".format(
...
def update_local_epochs(pattern, rank, rnd):
    ...
    if pattern == "uniform_random":
        np.random.seed(2021+rank+rnd+args.seed)
