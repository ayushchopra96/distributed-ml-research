    ...
    def __getNonIIDdata__(self, data, sizes, seed, alpha):
        ...
        for partPointer in range(len(partitions)):
            ...
            for labelIdx in requiredLabelList:
                ...
                idxIncrement = int(
                    basicLabelRatio*len(labelIdxDict[labelNameList[labelIdx]]))
                partitions[partPointer].extend(
                    labelIdxDict[labelNameList[labelIdx]][start:start + idxIncrement])
        ...
        for labelIdx in range(labelNum):
            remainLabels.extend(
                labelIdxDict[labelNameList[labelIdx]][labelIdxPointer[labelIdx]:])
...
def partition_dataset(rank, size, args):
    ...
    partition = DataPartitioner(
        trainset, partition_sizes, isNonIID=args.NIID, alpha=args.alpha)
