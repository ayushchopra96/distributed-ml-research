...
class Server(ServerModule):
    ...
    def train_clients(self):
        ...
        num_selection = int(
            round(self.args.num_clients*self.args.frac_clients))
        for curr_round in range(self.args.num_rounds*self.args.num_tasks):
            ...
            self.logger.print('server', 'round:{} train clients (selected_ids: {})'.format(
                curr_round, selected_ids))
            ...
            for clients in self.parallel_clients:
                ...
                for gid, cid in enumerate(clients):
                    ...
                    with tf.device('/device:GPU:{}'.format(gid)):
                        thrd = threading.Thread(target=self.invoke_client, args=(
                            client, cid, curr_round, selected, self.get_weights(cid), self.get_adapts()))
                ...
                for thrd in self.threads:
                    thrd.join()
            # update
            aggr = self.train.aggregate(
                self.updates, selected_ids, self.personalized_weights)

            # Update global weights only for clients which were selected
            for per_c in selected_ids:
                self.personalized_weights[per_c] = aggr[per_c]

        self.logger.print('server', 'done. ({}s)'.format(
            time.time()-self.start_time))
        sys.exit()

    def invoke_client(self, client, cid, curr_round, selected, weights, adapts):
        update = client.train_one_round(
            cid, curr_round, selected, weights, adapts)
        if not update == None:
    ...
    def get_adapts(self):
        if self.curr_round % self.args.num_rounds == 1 and not self.curr_round == 1:
            ...
            for lid, shape in enumerate(self.nets.shapes):
                shape = np.concatenate([self.nets.shapes[lid], [int(
                    round(self.args.num_clients*self.args.frac_clients))]], axis=0)
