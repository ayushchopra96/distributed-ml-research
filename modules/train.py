...
class TrainModule:
    ...
    def load_state(self, cid):
        self.state = np_load(os.path.join(
            self.args.state_dir, '{}_train.npy'.format(cid))).item()
        self.optimizer = tf.keras.optimizers.deserialize(
            self.state['optimizer'])

    def save_state(self):
        self.state['optimizer'] = tf.keras.optimizers.serialize(self.optimizer)
        np_save(self.args.state_dir, '{}_train.npy'.format(
            self.state['client_id']), self.state)
        ...
        if vlss < self.state['lowest_lss']:
            ...
            if self.state['curr_lr_patience'] <= 0:
                ...
                if self.state['curr_lr'] < self.args.lr_min:
                    self.logger.print(self.state['client_id'], 'epoch:%d, early-stopped as minium lr reached to %.5f' % (
                        self.state['curr_epoch'], self.state['curr_lr']))
        ...
        for epoch in range(self.args.num_epochs):
            ...
            for i in range(0, len(self.task['x_train']), self.args.batch_size):
                ...
                self.optimizer.apply_gradients(
                    zip(gradients, self.params['trainables']))
            self.validate()
            self.evaluate()
    ...
    def validate(self):
        ...
        for i in range(0, len(self.task['x_valid']), self.args.batch_size):
            ...
            self.add_performance('valid_lss', 'valid_acc',
                                 loss, y_batch, y_pred)
        self.vlss, self.vacc = self.measure_performance(
            'valid_lss', 'valid_acc')
        ...
        for tid in range(self.state['curr_task']+1):
            ...
            for i in range(0, len(x_test), self.args.batch_size):
                ...
                self.add_performance(
                    'test_lss', 'test_acc', loss, y_batch, y_pred)
    ...
    def calculate_capacity(self):
        def l1_pruning(weights, hyp):
            hard_threshold = np.greater(
                np.abs(weights), hyp).astype(np.float32)
    ...
    def calculate_communication_costs(self, params):
        ...
        for i, pruned in enumerate(params):
            actives = np.not_equal(
                pruned, np.zeros_like(pruned)).astype(np.float32)
            actives = np.sum(actives)
            num_actives += actives
    ...
    def aggregate(self, updates, selected_ids, personalized_weights):
    ...
    def client_similarity(self, client_weights, selected_ids, personalized_weights):
        sim_matrix = np.full(
            (
                self.args.num_clients,
                self.args.num_clients
            ),
            SELF_ATTENTION_CLIENT_WEIGHT
        )
        for j in range(self.args.num_clients):
            for i in range(j):
                if i in selected_ids:
                    ...
                    c1 = personalized_weights[i]
                if j in selected_ids:
                    c2 = client_weights[j]
                else:
                    c2 = personalized_weights[j]
