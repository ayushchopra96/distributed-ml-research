...
class ServerModule:
    ...
    def init_global_weights(self):
        self.personalized_weights = [
            self.nets.init_global_weights() for _ in range(self.args.num_clients)]
    ...
    def init_clients(self):
        ...
        if len(self.gpus) > 0:
            ...
            self.clients = {i: self.ClientObj(
                i, opt_copied, self.personalized_weights[i]) for i in range(num_parallel)}

    def get_weights(self, client_id):
        return self.personalized_weights[client_id]

    def set_weights(self, weights):
        self.personalized_weights = weights
...
class ClientModule:
    ...
    def load_state(self, cid):
        self.state = np_load(os.path.join(
            self.args.state_dir, '{}_client.npy'.format(cid))).item()
        self.update_train_config_by_tid(self.state['curr_task'])

    def save_state(self):
        np_save(self.args.state_dir, '{}_client.npy'.format(
    ...
    def update_train_config_by_tid(self, tid):
        ...
        self.trainable_variables_body = self.nets.get_trainable_variables(
            tid, head=False)
    ...
    def load_data(self):
        ...
        self.x_valid, self.y_valid = self.loader.get_valid(
            self.state['curr_task'])
        self.x_test_list, self.y_test_list = self.loader.get_test(
            self.state['curr_task'])
        self.train.set_task({
    ...
    def set_weights(self, weights):
        if self.args.model in ['fedweit']:
            ...
            for i, w in enumerate(weights):
                ...
                residuals = tf.cast(
                    tf.equal(w, tf.zeros_like(w)), dtype=tf.float32)
        ...
        if self.args.model in ['fedweit']:
            if self.args.sparse_comm:
                ...
                for lid, sw in enumerate(self.nets.decomposed_variables['shared']):
                    ...
                    thres = m_sorted[math.floor(
                        len(m_sorted)*(self.args.client_sparsity))]
                    m_bianary = tf.cast(tf.greater(
                        tf.abs(mask), thres), tf.float32).numpy().tolist()
