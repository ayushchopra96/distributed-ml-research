...
#     opt_bob = optim.SGD(model_bob.parameters(), lr=0.01, momentum=0.9, weight_decay=1e-4)
    opt_bob = optim.Adam(model_bob.parameters(), weight_decay=1e-4)
...
def experiment_ucb(
    ...
    for ep in t:  # 200
        ...
        if ep % 50 == 0 and ep > 0:
            if not os.path.isdir("runs/run12_tiny/"):
                os.mkdir("runs/run12_tiny/")
            torch.save(split_nn.state_dict(),
                       f"runs/run12_tiny/split_nn_{steps}")
            torch.save(interrupted_nn.state_dict(),
                       f"runs/run11_tiny/interrupted_nn_{steps}")
...
if __name__ == "__main__":
    ...
    interrupted_nn = get_model(
        num_clients=num_clients, interrupted=True, cifar=cifar)
