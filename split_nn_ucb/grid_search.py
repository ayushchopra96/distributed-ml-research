...
l1_weights = [0.1, 0.01, 0.05, 0.001, 0.005, 0.0001, 0.0005, 0.00001, 0.00005, 0.000001, 0.000005]
l1_weights = [f" --l1_norm_weight {item} " for item in l1_weights]

num_combinations = len(l1_weights) * 2 #* len(interrupt_ranges) * 2 * 2
...
def objective(trial):
    ...
    command = f"python3 train.py --cifar --use_random --interrupted --num_clients 5 --k 3 --non_iid_50 --use_masked --use_lenet --use_contrastive --experiment_name grid_search_{experiment_name} --epochs 20 --use_additive "
    ...
    os.system(command)   
    stats = json.load(open(f"./stats/grid_search_{experiment_name}5.json", "r"))
