...
def get_model(num_clients=100, use_masked=False, interrupted=False, avg=False):
    num_classes = 5
    option = 'A'
...
def experiment_ucb(
    ...
    miner = HardNegativeTripletMiner(0.5).to(device)  # AllTripletMiner(0.5).cuda()
    ...
    interrupted_nn.module.to(device)
...
if __name__ == "__main__":
    ...
    if hparams_.experiment_name == "":
        ...
        experiment_name = "non-iid-50_" + hparams_.experiment_name + str(hparams_.num_clients)
    ...
    cifar_train_loader_list, cifar_test_loader_list = get_non_iid_50(batch_size, 8, hparams_.num_clients)
    ...
    train_sizes = np.array(list(cifar_train_loader_list[0].ds_sizes.values()))
    interrupted_nn = get_model(
        num_clients=num_clients, use_masked=hparams_.use_masked, interrupted=interrupted)
