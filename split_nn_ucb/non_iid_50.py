...
class DataLoader:
    ...
    def get_valid(self, cid, task_id):
        valid = load_task(self.base_dir, self.task_set[cid][task_id]+'_valid.npy').item()
...
def get_non_iid_50(batch_size, num_workers, num_clients):
    ...
    for i in range(num_clients):
        ...
        ts_dl[i] = torch.utils.data.DataLoader(ts_ds, 256, num_workers=num_workers, collate_fn=non_iid_50_collate_fn, pin_memory=True)
    return tr_dl, ts_dl
