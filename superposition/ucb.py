...
class UCB:
    ...
    def update_clients(self, loss_mean, loss_std, mask):
        # # Assume loss statistics are discounted previous round's statistics
        # loss_mean = self.losses_mean[self.round_counter-1] * self.discount
        # loss_std = self.losses_mean[self.round_counter-1] * self.discount

        self.losses_mean[self.round_counter] = loss_mean.cpu().unsqueeze(0)
        self.losses_std[self.round_counter] = loss_std.cpu().unsqueeze(0)
        self.selection_mask[self.round_counter] = mask.cpu().unsqueeze(0)

    def end_round(self):
        zeros = torch.zeros((0, self.num_clients))
    ...
    def select_clients(self):
        A = advantage(
            torch.cat(self.losses_mean, 0),
            torch.cat(self.selection_mask, 0),
            self.discount,
            torch.cat(self.losses_std, 0),
            self.dataset_ratio
        )
...
if __name__ == "__main__":
    ...
    for r in range(num_rounds):
        l = torch.randn((num_clients, num_rounds)).cuda()
        m = l.mean(-1)
        s = l.std(-1)
        mask = m.bernoulli_().bool()
        # l = torch.zeros((dataset_sizes[0],))
        # print(c in selected_ids)
        b.update_clients(m, s, mask)
