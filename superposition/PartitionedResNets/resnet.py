...
from .layers import ParitionedLinear, PartitionedBatchNorm2d, PartitionedConv2d
...
def _weights_init(m):
    ...
    # print(classname)
    if isinstance(m, ParitionedLinear) or isinstance(m, PartitionedConv2d):
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_partitions, num_clients, num_classes=10, hooked=False, option='A'):
        ...
        self.linear = ParitionedLinear(64, num_classes, num_partitions)

        self.context_weights = nn.Parameter(
            torch.randn(num_partitions, num_clients))
